    ...
    <div className="animate-spring-up overflow-x-hidden">
      <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">System <span className="text-slate-400">Intelligence</span></h2>
        </div>
        
        <div className="flex items-center gap-3">
            <button className="p-3 bg-white border border-slate-200 text-slate-400 rounded-2xl hover:text-blue-600 hover:border-blue-200 transition-all">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-50 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            <div>
                 ...
                 <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Live service feedback</p>
            </div>
            <button onClick={() => navigate('/admin/appointments')} className="w-10 h-10 bg-slate-50 text-slate-400 hover:bg-blue-600 hover:text-white rounded-full flex items-center justify-center transition-all">
               <ArrowRight size={18} />
            </button>
          </div>
          ...
          <div className="space-y-4">
            {appointments.length === 0 ? (
                <p className="text-center py-10 text-slate-400 font-bold uppercase text-xs tracking-widest">No recent data</p>
            ) : (
                  ...
                  <div key={appt._id} className="flex items-center justify-between p-5 rounded-[1.5rem] bg-slate-50/50 hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all group border border-transparent hover:border-slate-100">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{appt.serviceType}</p>
                       </div>
                    </div>
                    <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                      (appt.serviceStatus || appt.status) === 'COMPLETED' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                    }`}>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-50 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            <div>
                 ...
                 <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Role-based user health</p>
            </div>
          ...
          <div className="mt-12 bg-slate-900 rounded-[2rem] p-6 text-white flex items-center justify-between">
             <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Platform Meta</p>
